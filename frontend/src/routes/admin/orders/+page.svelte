...
<section class="space-y-6">
			...
			<article class="panel space-y-4">
				<div class="flex flex-wrap items-center justify-between gap-4">
					<div>
						...
						<p class="text-sm text-slate-500">{order.buyer?.name ?? 'Unknown buyer'} · {order.status}</p>
					</div>
					...
					{/if}
						{#if order.status === 'refunding'}
							<form method="POST" action="?/refund">
								<input name="order_id" type="hidden" value={order.id} />
								<button class="rounded-full border border-amber-300 px-4 py-2 text-sm text-amber-700" type="submit">Approve refund</button>
							</form>
						{/if}
					</div>
				</div>
	...
	<Pagination basePath="/admin/orders" meta={data.orders.meta} />
</section>
