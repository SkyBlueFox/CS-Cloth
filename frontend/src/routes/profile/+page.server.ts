...
export const actions: Actions = {
    updateProfile: async (event) => {
        ...
        if (user.role === 'admin') {
            return fail(403, { error: 'Admins cannot update their profile.' });
        }
    ...
    saveAddress: async (event) => {
        ...
        try {
            ...
            return fail(422, { addressError: getErrorMessage(error, 'Unable to save address.') });
        }

        return { addressSuccess: 'Address saved.' };
    },
    deleteAddress: async (event) => {
        requireUser(event, ['user']);
        const form = await event.request.formData();
        try {
            await backend(event, `/addresses/${form.get('address_id')}`, { method: 'DELETE' });
        } catch (error) {
            return fail(422, { addressError: getErrorMessage(error, 'Unable to delete address.') });
        }
        return { addressSuccess: 'Address deleted.' };
    }
};