<script lang="ts">
    ...
    // ดึงค่า wallet ออกมาเพื่อใช้ง่ายขึ้น พร้อมค่า Default ป้องกัน error
    let wallet = $derived(data.wallet || { balance: 0, transactions: { data: [] } });

    const presetAmounts = [100, 300, 500, 1000, 2000];
    const providers = [
        { group: 'mobile_banking', value: 'scb_easy', label: 'SCB EASY', note: 'Instant confirmation via app', logo: scbEasyLogo },
        { group: 'mobile_banking', value: 'k_plus', label: 'K PLUS', note: 'Kasikorn Bank mobile app', logo: kPlusLogo },
        { group: 'mobile_banking', value: 'krungthai_next', label: 'Krungthai NEXT', note: 'Krungthai Bank mobile app', logo: krungthaiNextLogo },
        { group: 'mobile_banking', value: 'krungsri_app', label: 'Krungsri Mobile App', note: 'Krungsri mobile banking', logo: krungsriLogo },
        { group: 'mobile_banking', value: 'bualuang_mbanking', label: 'Bangkok Bank Mobile Banking', note: 'Bangkok Bank mobile app', logo: bangkokBankLogo },
        { group: 'wallet', value: 'line_pay', label: 'LINE Pay', note: 'Pay with your LINE Pay wallet', logo: linePayLogo },
        { group: 'wallet', value: 'apple_pay', label: 'Apple Pay', note: 'Pay with Apple Pay', logo: applePayLogo },
        { group: 'wallet', value: 'truemoney', label: 'TrueMoney', note: 'Pay with TrueMoney Wallet', logo: trueMoneyLogo },
        { group: 'wallet', value: 'google_pay', label: 'Google Pay', note: 'Pay with Google Pay', logo: googlePayLogo },
        { group: 'wallet', value: 'shopee_pay', label: 'ShopeePay', note: 'Pay with ShopeePay', logo: shopeePayLogo },
        { group: 'card', value: 'credit_debit_card', label: 'Credit / Debit Card', note: 'Pay using your credit or debit card details.', logo: visaLogo, extraLogos: [mastercardLogo, unionPayLogo, jcbLogo, americanExpressLogo] }
    ];

    const mobileBankingProviders = providers.filter((p) => p.group === 'mobile_banking');
    const alternateProviders = providers.filter((p) => p.group !== 'mobile_banking');
    ...
    function customAmountChanged(event: Event) {
        ...
        if (customAmount !== '') selectedPresetAmount = '';
    }

    function cardNumberChanged(event: Event) {
        const input = event.currentTarget as HTMLInputElement;
        const digits = input.value.replace(/\D+/g, '').slice(0, 16); // บัตรเครดิตส่วนใหญ่ 16 หลัก
...
<section class="mx-auto max-w-7xl space-y-8">
    ...
    {#if data.error || form?.error || form?.success}
        <div class="rounded-[1.5rem] shadow-lg overflow-hidden">
            {#if data.error}
                <p class="bg-rose-50 px-8 py-5 text-sm font-black text-rose-800 ring-1 ring-rose-200">{data.error}</p>
            {:else if form?.error}
    ...
    <div class="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                {#each presetAmounts as amount (amount)}
                    <label class="group relative cursor-pointer">
                        ...
                        >
                            <span class={`text-[10px] font-black uppercase tracking-widest ${selectedPresetAmount === String(amount) ? 'text-blue-600' : 'text-slate-400'}`}>
                                Quick
                            </span>
                    ...
                    {#each mobileBankingProviders as provider (provider.value)}
                        <label class="group relative cursor-pointer">
                            ...
                            <div class={`flex items-center gap-5 rounded-[1.5rem] border-2 p-5 transition-all ${selectedProvider === provider.value ? 'border-blue-600 bg-blue-50/50 shadow-lg' : 'border-slate-50 bg-white hover:border-slate-200'}`}>
                                ...
                                <div class={`h-6 w-6 rounded-full border-2 border-slate-200 ${selectedProvider === provider.value ? 'border-blue-600 bg-blue-600 ring-4 ring-blue-100' : 'group-hover:border-blue-300'}`}></div>
                            </div>
                        </label>
                    {/each}
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid gap-3">
                    {#each alternateProviders as provider (provider.value)}
                        <label class="group relative cursor-pointer">
                            <input class="sr-only" name="provider" type="radio" value={provider.value} bind:group={selectedProvider} />
                            <div class={`flex items-start gap-5 rounded-[1.5rem] border-2 p-5 transition-all ${selectedProvider === provider.value ? 'border-blue-600 bg-blue-50/50 shadow-lg' : 'border-slate-50 bg-white hover:border-slate-200'}`}>
                                <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white p-2 shadow-sm ring-1 ring-slate-100">
                                ...
                                <div class="flex-1">
                                    ...
                                    {#if provider.extraLogos}
                                        <div class="mt-3 flex flex-wrap items-center gap-2">
                                            {#each provider.extraLogos as extraLogo (extraLogo)}
            ...
            {#if selectedProvider === 'credit_debit_card'}
                <div class="grid gap-5 rounded-[2rem] border border-slate-200 bg-slate-50 p-6 shadow-sm ring-1 ring-slate-900/5">
                    ...
                    <label class="block">
                        ...
                        <input class="w-full" name="cardholder_name" bind:value={cardholderName} placeholder="JOHN DOE" />
                    </label>
                    <label class="block">
                        <span class="text-slate-700">Card Number</span>
                        <input class="w-full tracking-[0.2em]" name="card_number" bind:value={cardNumber} oninput={cardNumberChanged} placeholder="0000 0000 0000 0000" />
                    </label>
                    <div class="grid grid-cols-2 gap-5">
                        <label class="block">
                            <span class="text-slate-700">Expiry Date</span>
                            <input class="w-full text-center" name="expiry_date" bind:value={expiryDate} oninput={expiryDateChanged} placeholder="MM/YY" />
                        </label>
                        <label class="block">
                            <span class="text-slate-700">CVV</span>
                            <input class="w-full text-center" name="cvv" bind:value={cvv} oninput={cvvChanged} placeholder="CVV" />
                        </label>
                    </div>
                </div>
            {/if}
        ...
        <div class="panel h-full flex flex-col">
            ...
            {#if !wallet.transactions?.data || wallet.transactions.data.length === 0}
            ...
            {:else}
                <div class="space-y-4">
                    {#each wallet.transactions.data as transaction (transaction.id)}
                        <div class="rounded-3xl border border-slate-100 bg-white p-6 transition-all hover:border-blue-100 hover:shadow-xl hover:shadow-blue-900/5">
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="text-sm font-black uppercase tracking-tight text-slate-900">{transaction.type?.replace('_', ' ') || 'TOP UP'}</p>
                            ...
                            <div class="mt-6 flex justify-between border-t border-slate-50 pt-4 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                                <span>Balance: ฿{transaction.balance_before?.toLocaleString() ?? 0}</span>
                                <span class="text-blue-600">→ ฿{transaction.balance_after?.toLocaleString() ?? 0}</span>
                            </div>
                        </div>
                    {/each}
                </div>
