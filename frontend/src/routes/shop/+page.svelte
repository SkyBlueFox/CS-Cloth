...
<section class="flex flex-col gap-12">
    <header class="relative overflow-hidden rounded-[3rem] bg-white border border-slate-200 p-10 sm:p-16 shadow-sm">
        <div class="relative z-10 flex flex-col gap-5">
            ...
            <p class="max-w-lg text-base font-bold leading-relaxed text-slate-600 uppercase tracking-wide">
    ...
    <div class="grid grid-cols-1 gap-10 md:grid-cols-2 xl:grid-cols-3">
        {#each data.items.data as item (item.id)}
            <a 
                class="group relative flex flex-col rounded-[3rem] border border-transparent bg-white p-4 shadow-sm transition-all duration-500 hover:border-blue-100 hover:shadow-2xl hover:shadow-blue-900/10 hover:-translate-y-2" 
                href={`/shop/${item.id}`}
            ...
            >
                <div class="relative aspect-[4/3] w-full overflow-hidden rounded-[2.5rem] bg-slate-100 shadow-inner">
                    ...
                    <div class="absolute left-6 top-6 rounded-full bg-white/90 px-4 py-1.5 text-[10px] font-black text-slate-900 shadow-xl backdrop-blur-md ring-1 ring-black/5">
                        AVAIL: {item.stock}
                    </div>
                </div>
                ...
                <div class="flex flex-1 flex-col px-4 py-8">
                    <div class="mb-8 flex-1">
                        <h2 class="text-2xl font-black text-slate-900 transition-colors group-hover:text-blue-700 tracking-tight">
                            {item.name}
                        </h2>
                        <p class="mt-3 line-clamp-2 text-sm font-bold leading-relaxed text-slate-500 uppercase tracking-tight">
                            {item.description}
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-between border-t border-slate-50 pt-8">
                        ...
                        <div class="flex h-14 w-14 items-center justify-center rounded-[1.5rem] bg-slate-900 text-white shadow-xl shadow-slate-900/20 transition-all duration-300 group-hover:bg-blue-600 group-hover:shadow-blue-600/40 group-hover:rotate-12">
