...
export const load = async (event) => {
    try {
        // แก้ไข: เอาคำว่า /shop ออก ให้ตรงกับ Laravel route -> /items/{item}
        const response = await backend<{ item: Item; questions: Question[] }>(
            event,
            `/items/${event.params.id}`
        );

        let addressesData: Address[] = [];
        
        // ดึงข้อมูลที่อยู่เฉพาะเมื่อเป็น User
...
export const actions = {
    question: async (event) => {
        ...
        try {
            // แก้ไข: เอาคำว่า /shop ออก -> /items/{item}/questions
    ...
    order: async (event) => {
        ...
        try {
            ...
            // แก้ไข: ยิงไปที่ /orders ตามที่ Laravel กำหนด และแนบ item_id ไปใน body ด้วย
            await backend(event, `/orders`, {
                method: 'POST',
                body: {
                    item_id: Number(event.params.id), // ต้องส่ง ID สินค้าไปด้วยเพราะ Route เปลี่ยน
